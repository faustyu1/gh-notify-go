...
func (r repoDetail) Render(_ context.Context, s ui.Session) (ui.View, error) {
	...
	// A webhook-connected repository may live on any instance, so its
	// address travels in the params rather than being built from the name.
	provider := s.Params["provider"]
	if provider == "" {
		provider = domain.ProviderGitHub
	}
	open := ui.Button{Label: l.T("btn.open_on", "name", forge.Name(provider)),
		Icon: render.EmojiLink, URL: s.Params["url"]}
	if provider == domain.ProviderGitHub {
		open.URL = "https://github.com/" + name
	}
