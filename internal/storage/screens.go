...
// DeleteChat removes a chat the bot was removed from, cascading to its
// integrations, managers and topics. Leaving the rows would keep the chat in
// the picker and the chats screen, which reads as "the bot still knows about
// this chat".
func (s *Store) DeleteChat(ctx context.Context, telegramChatID int64) error {
	_, err := s.pool.Exec(ctx,
		`DELETE FROM chats WHERE telegram_chat_id = $1`, telegramChatID)
	if err != nil {
...
// was kicked and re-added would stay silent forever.
