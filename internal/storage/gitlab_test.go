...
func TestGitLabConnectionTokenRoundTrip(t *testing.T) {
	...
	id, err := store.CreateGitLabConnection(ctx, userID)
	require.NoError(t, err)

	token, err := store.GitLabWebhookToken(ctx, id, userID)
	require.NoError(t, err)
	require.Len(t, token, 64)

	found, err := store.GitLabInstallationByToken(ctx, token)
	require.NoError(t, err)
...
func TestGitLabTokenIsOwnerOnly(t *testing.T) {
	...
	id, err := store.CreateGitLabConnection(ctx, owner)
	require.NoError(t, err)

	_, err = store.GitLabWebhookToken(ctx, id, other)
	require.Error(t, err)
}
...
func TestGitLabUnknownTokenIsRefused(t *testing.T) {
	...
	_, err := store.GitLabInstallationByToken(ctx, "nope")
	require.ErrorIs(t, err, storage.ErrUnknownWebhookToken)
	_, err = store.GitLabInstallationByToken(ctx, "")
	require.ErrorIs(t, err, storage.ErrUnknownWebhookToken)
}
...
func TestGitLabConnectionForSetupReusesUnusedOne(t *testing.T) {
	...
	first, err := store.GitLabConnectionForSetup(ctx, userID)
	require.NoError(t, err)
	again, err := store.GitLabConnectionForSetup(ctx, userID)
	require.NoError(t, err)
	require.Equal(t, first, again, "an unused connection must be reused")

	require.NoError(t, store.RememberGitLabProject(ctx, first,
		storage.GitLabProject{ID: 15, Path: "mike/diaspora"}))
	next, err := store.GitLabConnectionForSetup(ctx, userID)
	require.NoError(t, err)
...
func TestRememberGitLabProjectNamesConnection(t *testing.T) {
	...
	require.NoError(t, store.RememberGitLabProject(ctx, id, storage.GitLabProject{
...
func TestGitLabIntegrationsDoNotCollideWithGitHub(t *testing.T) {
	...
	gitlabInst, err := store.CreateGitLabConnection(ctx, userID)
	require.NoError(t, err)
	...
	found, err := store.IntegrationsForGitLabProject(ctx, gitlabInst, 42)
	require.NoError(t, err)
...
func TestDeleteGitLabConnectionIsOwnerOnlyAndCascades(t *testing.T) {
	...
	require.NoError(t, store.DeleteGitLabConnection(ctx, id, other))
	_, err = store.InstallationByID(ctx, id)
	require.NoError(t, err, "someone else's delete must be a no-op")

	require.NoError(t, store.DeleteGitLabConnection(ctx, id, owner))
	_, err = store.InstallationByID(ctx, id)
	require.Error(t, err)
